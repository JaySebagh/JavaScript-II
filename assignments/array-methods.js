...
// because the director BECAME DRUNK WITH POWER. Convert each first name into all caps
// and log the result
let allCaps = [];

allCaps = runners.map(function(newNames) {
  let firstNames = newNames.first_name.toUpperCase();
  return firstNames
});
...
largeShirts = runners.filter(big => big.shirt_size === "L");
...
runners.forEach(function(newDono) {
  let theDono = newDono.donation;
  ticketPriceTotal.push(theDono)
});
...
// I want you to think of potential problems you could solve given the data set and
//the 5k fun run theme.  Try to create and then solve 3 unique problems using one or
//many of the array methods listed above.

// Problem 1

// We need to know how many people are size S
let smallShirts = [];

smallShirts = runners.filter(small => small.shirt_size === "S");

console.log(smallShirts);

// Problem 2

// We need to message everyone that their item has been shipped. Get all the emails.

let emailList = [];

runners.forEach(function(allEmails) {
  let listEmails = allEmails.email;
  emailList.push(listEmails)
});
...
// We want to send a special thanks to anyone who donated over 200.
//  Get a list including people who donated 200+

let donoTwoHundred = [];

donoTwoHundred = runners.filter(dono => dono.donation >= 200);

console.log(donoTwoHundred);